...
vector<vector<int> > // A: часто добавляют пробел чтобы не было путаницы с оператором >> 
one_step(vector<vector<int>>& board, char* dx, char* dy, int nMoves, 
...
vector<vector<int>> steps(vector<vector<int> >& board, char* dx, char* dy, 
	const vector<vector<int> >& cells, int M) 
{
	/*
	...
	*/
	...
	{
		vector<vector<int> > new_cells = one_step(board, dx, dy, cells[i][0], cells[i][1]); 
		//
		for (int j = 0; j < new_cells.size(); j++)
		{
			...
			if  (
					!(
						find(all_new_cells.begin(), all_new_cells.end(), new_cells[j]) != all_new_cells.end()
				))
...
void test_task_1_2() 
{
	...
	int x0 = 2, y0 = 2; 
	cout << "Cells reachable from the corner (" << x0 << "," << y0 << ") in two steps" << endl;
	task_1_2(x0, y0, 2);
	cout << endl;
}
